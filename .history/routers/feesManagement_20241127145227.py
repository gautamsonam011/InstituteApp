...
async def create_class_fee_head(
    ...
    return {"message":"Class Fee Head Added Successfully"}
...
async def update_class_fee_head(
    ...
    return {"message":"Class Fee Head Edited Successfully"}
...
async def get_class_fee_head(
    ...
    if current_user.get("association").get("level") > 1:
        raise HTTPException(status_code=status.HTTP_405_METHOD_NOT_ALLOWED,
                            detail="You are not allowed to change class fee head"
                            )
...
async def delete_class_fee_head(
    ...
    if not data:
        raise HTTPException(status_code=404, detail= "Class Fee Head not found")
   
    db.delete(data)
    db.commit()

    return {"message": "Class Fee Head deleted successfully"}

# -------------------- Fee Submission Details ------------->


@router.post("/feeSubmission")
async def create_fee_submission(
    request: fee_submission_details,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    if not current_user.get("association") or not current_user.get("association").get("branchID"):
    ...
    new_data = FeeSubmissionDetails(
        studentNo = request.studentNo,
        courseName = request.courseName,
        studentName = request.studentName,
        totalDueAmount = request.totalDueAmount,
        paymentMode = request.paymentMode,
        receiptNo = request.receiptNo,
        amountToSubmit = request.amountToSubmit,
        status = "Completed",
        user_id=current_user["id"],
        headOfficeID=current_user.get("association").get("headOfficeID"),
        branchID=current_user.get("association").get("branchID"),
    ...
    return {"message":"Fee Submission Added Successfully"}

@router.put("/feeSubmission/{fee_id}")
async def update_fee_submission(
    fee_id: int,
    request: fee_submission_details,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
    ...
    # Retrieve the fee from the database
    fee_submissions = db.query(FeeSubmissionDetails).filter(FeeSubmissionDetails.id == fee_id
                                                      ).filter(FeeSubmissionDetails.branchID == current_user.get("association").get("branchID")).first()

    if fee_submissions is None:
        raise HTTPException(status_code=404, detail="Fee Submission not found")

    # Update the fee fields
    fee_submissions.studentNo = request.studentNo
    fee_submissions.courseName = request.courseName
    fee_submissions.studentName = request.studentName
    fee_submissions.totalDueAmount = request.totalDueAmount
    fee_submissions.paymentMode = request.paymentMode
    fee_submissions.receiptNo = request.receiptNo
    fee_submissions.amountToSubmit = request.amountToSubmit
    fee_submissions.status = "Edited"
    fee_submissions.user_id = current_user.get("id")
    fee_submissions.headOfficeID = current_user.get("association").get("headOfficeID")
    fee_submissions.branchID = current_user.get("association").get("branchID")
    fee_submissions.level = current_user.get("association").get("level")

    # Commit the changes to the database
    db.commit()
    db.refresh(fee_submissions)

    return {"message":"Fee Submission Edited Successfully"}

@router.get("/feeSubmission/{fee_id}")
async def get_fee_submission(
    fee_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)):
    ...
    if current_user.get("association").get("level") > 1:
        raise HTTPException(status_code=status.HTTP_405_METHOD_NOT_ALLOWED,
                            detail="You are not allowed to change fee submission"
                            )
    
    new_data = db.query(FeeSubmissionDetails).filter(
        FeeSubmissionDetails.id == fee_id,
        FeeSubmissionDetails.branchID == current_user.get("association").get("branchID")
        ).first()
    
    return new_data

@router.get("/feeSubmissionAll")
async def get_all_fee_submission(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)):

    if current_user.get("association").get("level") > 1:
        raise HTTPException(status_code=status.HTTP_405_METHOD_NOT_ALLOWED,
                            detail="You are not allowed to change submission"
                            )
...
@router.delete("/feeSubmission/{fee_id}")
async def delete_fee_submission(
    fee_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    ...
    data = db.query(FeeSubmissionDetails).filter(
        FeeSubmissionDetails.id == fee_id,
        FeeSubmissionDetails.branchID == current_user.get("association").get("branchID")
    ).first()
    ...
    return {"message": "Fee Submission deleted successfully"}
